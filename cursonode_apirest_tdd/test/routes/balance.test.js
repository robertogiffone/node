...
describe('Ao calcular o saldo do usuário...', () => {
  ...
  test('Deve considerar transação passada', () => {
    return request(app).post(ROUTE_TRANSACTION)
      .send({
        description: '1', date: dayjs().subtract(5, 'day'), ammount: 250, type: 'I', acc_id: 10100, status: true,
      })
      .set('authorization', `bearer ${TOKEN}`)
      .then(() => {
        return request(app).get(MAIN_ROUTE)
  ...
  test('Não deve considerar transação futura', () => {
    return request(app).post(ROUTE_TRANSACTION)
      .send({
        description: '1', date: dayjs().add(5, 'day'), ammount: 250, type: 'I', acc_id: 10100, status: true,
      })
      .set('authorization', `bearer ${TOKEN}`)
      .then(() => {
        return request(app).get(MAIN_ROUTE)
